...
static bongocat_error_t validate_config(config_t *config) {
  ...
  if (config->cat_height < MIN_CAT_HEIGHT ||
      config->cat_height > MAX_CAT_HEIGHT) {
    bongocat_log_warning("cat_height %d out of range [%d-%d], clamping",
                         config->cat_height, MIN_CAT_HEIGHT, MAX_CAT_HEIGHT);
    config->cat_height =
        config->cat_height < MIN_CAT_HEIGHT ? MIN_CAT_HEIGHT : MAX_CAT_HEIGHT;
  }

  if (config->overlay_height < MIN_OVERLAY_HEIGHT ||
      config->overlay_height > MAX_OVERLAY_HEIGHT) {
    bongocat_log_warning("overlay_height %d out of range [%d-%d], clamping",
                         config->overlay_height, MIN_OVERLAY_HEIGHT,
                         MAX_OVERLAY_HEIGHT);
    config->overlay_height = config->overlay_height < MIN_OVERLAY_HEIGHT
                                 ? MIN_OVERLAY_HEIGHT
                                 : MAX_OVERLAY_HEIGHT;
  }
  ...
  if (config->fps < MIN_FPS || config->fps > MAX_FPS) {
    bongocat_log_warning("fps %d out of range [%d-%d], clamping", config->fps,
                         MIN_FPS, MAX_FPS);
    config->fps = config->fps < MIN_FPS ? MIN_FPS : MAX_FPS;
  }

  // Validate durations
  if (config->keypress_duration < MIN_DURATION ||
      config->keypress_duration > MAX_DURATION) {
    bongocat_log_warning("keypress_duration %d out of range [%d-%d], clamping",
                         config->keypress_duration, MIN_DURATION, MAX_DURATION);
    config->keypress_duration =
        config->keypress_duration < MIN_DURATION ? MIN_DURATION : MAX_DURATION;
  }

  if (config->test_animation_duration < MIN_DURATION ||
      config->test_animation_duration > MAX_DURATION) {
    bongocat_log_warning(
        "test_animation_duration %d out of range [%d-%d], clamping",
        config->test_animation_duration, MIN_DURATION, MAX_DURATION);
    config->test_animation_duration =
        config->test_animation_duration < MIN_DURATION ? MIN_DURATION
                                                       : MAX_DURATION;
  }
  ...
  if (config->overlay_opacity < 0 || config->overlay_opacity > 255) {
    bongocat_log_warning("overlay_opacity %d out of range [0-255], clamping",
    ...
    if (sscanf(line, " %255[^=] = %255s", key, value) == 2) {
      ...
      if (strcmp(key_start, "cat_x_offset") == 0) {
        ...
        if (strcmp(value, "top") == 0) {
          ...
          bongocat_log_warning("Invalid overlay_position '%s', using 'top'",
                               value);
          config->overlay_position = POSITION_TOP;
        }
      } else if (strcmp(key_start, "keyboard_device") == 0 ||
                 strcmp(key_start, "keyboard_devices") == 0) {
        // Reallocate device array
        config_keyboard_devices = realloc(
            config_keyboard_devices, (config_num_devices + 1) * sizeof(char *));
        if (!config_keyboard_devices) {
          ...
          config_keyboard_devices[config_num_devices] =
              BONGOCAT_MALLOC(value_len + 1);
          if (config_keyboard_devices[config_num_devices]) {
            strncpy(config_keyboard_devices[config_num_devices], value,
                    value_len);
            ...
            bongocat_log_error(
                "Failed to allocate memory for keyboard_device entry");
            result = BONGOCAT_ERROR_MEMORY;
          }
        }
      } else {
        bongocat_log_warning("Unknown configuration key '%s' at line %d",
                             key_start, line_number);
      }
...
bongocat_error_t load_config(config_t *config, const char *config_file_path) {
  ...
  *config =
      (config_t){.screen_width = get_screen_width(),
  ...
  if (config_num_devices == 0) {
    ...
    if (config_keyboard_devices) {
      config_keyboard_devices[0] =
          BONGOCAT_MALLOC(strlen("/dev/input/event4") + 1);
  ...
  bongocat_log_debug("  Cat: %dx%d at offset (%d,%d)", config->cat_height,
                     (config->cat_height * 954) / 393, config->cat_x_offset,
                     config->cat_y_offset);
