...
bongocat_error_t animation_init(config_t *config) {
  ...
  const unsigned char *embedded_data[] = {
      bongo_cat_both_up_png, bongo_cat_left_down_png, bongo_cat_right_down_png};

  const size_t embedded_sizes[] = {bongo_cat_both_up_png_size,
                                   bongo_cat_left_down_png_size,
                                   bongo_cat_right_down_png_size};

  const char *frame_names[] = {"embedded bongo-cat-both-up.png",
  ...
  for (int i = 0; i < NUM_FRAMES; i++) {
    ...
    anim_imgs[i] =
        stbi_load_from_memory(embedded_data[i], embedded_sizes[i],
