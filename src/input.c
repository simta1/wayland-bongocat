...
static void capture_input_multiple(char **device_paths, int num_devices,
  ...
  for (int i = 0; i < num_devices; i++) {
    ...
    if (fds[i] < 0) {
      bongocat_log_warning("Failed to open %s: %s", device_paths[i],
                           strerror(errno));
      continue;
    }

    bongocat_log_info("Input monitoring started on %s (fd=%d)", device_paths[i],
                      fds[i]);
    if (fds[i] > max_fd) {
      max_fd = fds[i];
    }
    ...
    for (int i = 0; i < num_devices; i++) {
      if (fds[i] >= 0 && FD_ISSET(fds[i], &readfds)) {
        ...
        if (rd < 0) {
          if (errno == EAGAIN)
            continue;
          bongocat_log_warning("Read error on %s: %s", device_paths[i],
                               strerror(errno));
          close(fds[i]);
          fds[i] = -1;
          valid_devices--;
          ...
          if (ev[j].type == EV_KEY && ev[j].value == 1) {
            ...
            if (enable_debug) {
              bongocat_log_debug(
                  ...
                  device_paths[i], ev[j].code, ev[j].time.tv_sec,
                  ev[j].time.tv_usec);
            }
          }
        }
